...
goalNode = depth_first_search(gp)
...
goalNode = breadth_first_search(gp)
...
def ucs_search(problem, path_cost=0, pop_index=0):
    node = Node(problem.initial)
    ...
    frontier.sortAppend(node, path_cost)
    ...
    while frontier:
        ...
        for child in node.expand(problem):
            ...
            if child.state not in explored and child not in frontier:
                frontier.sortAppend(child, path_cost)

    return None


def gbfs_search(problem, heuristicSLD=0, pop_index=0):
    node = Node(problem.initial)
    ...
    frontier.sortAppend(node, heuristicSLD)
    ...
    while frontier:
        ...
        for child in node.expand(problem):
            ...
            if child.state not in explored and child not in frontier:
                frontier.sortAppend(child, heuristicSLD)
